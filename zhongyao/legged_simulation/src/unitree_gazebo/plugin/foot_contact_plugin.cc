    ...
    class UnitreeFootContactPlugin : public SensorPlugin
    {
        ...
        void Load(sensors::SensorPtr _sensor, sdf::ElementPtr _sdf)
        {
            ...
            if (_sdf->HasElement("frame_id")) {
                this->frame_id = _sdf->Get<std::string>("frame_id");
            } else {
                this->frame_id = _sensor->Name();  // 默认使用传感器名称作为 frame_id
            }
        ...
        void OnUpdate()
        {
            ...
            for (unsigned int i = 0; i < count; ++i){
                ...
                //     ROS_INFO("Contact %d has %d positions.", i, contacts.contact(i).position_size());
                //     // continue;
                // }     
            ...
            if(count != 0){           
                ...
                // ROS_INFO("Force: %f, %f, %f", Fx, Fy, force.wrench.force.z);
                count = 0;
                Fx = 0;
                Fy = 0;
                Fz = 0;
            ...
            force.header.frame_id = this->frame_id;
