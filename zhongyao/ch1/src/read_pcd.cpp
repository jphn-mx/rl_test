...
void cloudCallback(const sensor_msgs::PointCloud2ConstPtr &input)
{
    ...
    {
        if (cloud_filtered->points[i].x < min_y_point.x)
        {
            min_y_point = cloud_filtered->points[i];
        }
    }

    ROS_INFO("Point with the minimum x value: [%f, %f, %f]", min_y_point.x, min_y_point.y, min_y_point.z);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub = nh.subscribe("/viobot/tof_cloud", 1, cloudCallback);
